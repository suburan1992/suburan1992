...
//ui components
import AddDomainCatagory from "./forms/AddDomainCatagory";
import EditDomainCAtagory from "./forms/EditDomainCAtagory";
import TableDomainCatagory from "./tables/TableDomainCatagory";
...
export default function ManageDomainCatagory() {
  ...
  const updateDesignation = (id, designation) => {
    setData(false);
    axios
      .put("http://localhost:8090/api/DomainCategory/" + id, designation)
      .then((res) => {
        if (res.status === 200) {
          setEditing(false);
          setPreview(true);
  ...
  const addDesignation = (designation) => {
    setData(false);
    axios
      .post("http://localhost:8090/api/DomainCategory/", designation)
      .then((res) => {
        console.log(res);
        setPreview(true);
        setData(true);
  ...
  const deleteDesignation = (row) => {
    ...
    if (k) {
      axios
        .delete("http://localhost:8090/api/DomainCategory/" + row.id)
        .then((res) => {
          setData(true);
          if (res.status === 200) {
            setState({
        ...
        <div className="container-fluid">
          <div className="inner-body">
            ...
            <Grid container>
              ...
              >
                <Typography variant="h6">Domain Category</Typography>
                {preview ? (
                  <Typography variant="body2">
                    Masters / Domain Category / Domain Category List
                  </Typography>
                ) : editing ? (
                  <Typography variant="body2">
                    Masters / Domain Category / Edit Domain Category
                  </Typography>
                ) : (
                  <Typography variant="body2">
                    Masters / Domain Category / Add Domain Category
                  </Typography>
                )}
              </Grid>
              <Grid
            ...
            <Grid container>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                {preview ? (
                  ...
                  <AddDomainCatagory addDesignation={addDesignation} />
                )}
              </Grid>
            </Grid>
          </div>
