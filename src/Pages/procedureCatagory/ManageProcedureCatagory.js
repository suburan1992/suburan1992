...
//ui components
import AddProcedureCatagory from "./forms/AddProcedureCatagory";
import EditProcedureCatagory from "./forms/EditProcedureCatagory";
import TableProcedureCatagory from "./tables/TableProcedureCatagory";
...
export default function ManageProcedureCatagory() {
  ...
  const updateDesignation = (id, designation) => {
    setData(false);
    axios
      .put("http://localhost:8090/api/ProcedureCategory/" + id, designation)
      .then((res) => {
        if (res.status === 200) {
          setEditing(false);
          setPreview(true);
  ...
  const addDesignation = (designation) => {
    setData(false);
    axios
      .post("http://localhost:8090/api/ProcedureCategory/", designation)
      .then((res) => {
        setPreview(true);
        setData(true);
        if (res.status === 200) {
  ...
  const deleteDesignation = (row) => {
    ...
    if (k) {
      axios
        .delete("http://localhost:8090/api/ProcedureCategory/" + row.id)
        .then((res) => {
          setData(true);
          if (res.status === 200) {
            setState({
        ...
        <div className="container-fluid">
          <div className="inner-body">
            ...
            <Grid container>
              ...
              >
                <Typography variant="h6">Procedure Category</Typography>
                {preview ? (
                  <Typography variant="body2">
                    Masters / Procedure Category / Procedure Category List
                  </Typography>
                ) : editing ? (
                  <Typography variant="body2">
                    Masters / Procedure Category / Edit Procedure Category
                  </Typography>
                ) : (
                  <Typography variant="body2">
                    Masters / Procedure Category / Add Procedure Category
                  </Typography>
                )}
              </Grid>
              <Grid
            ...
            <Grid container>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                {preview ? (
                  <TableProcedureCatagory
                    rows={rows}
                    editRow={editRow}
                    deleteDesignation={deleteDesignation}
                  />
                ) : editing ? (
                  < EditProcedureCatagory
                    editing={editing}
                  ...
                  <AddProcedureCatagory addDesignation={addDesignation} />
                )}
              </Grid>
            </Grid>
          </div>
