...
import TableDesignation from "./tables/TableDesignation";
...
export default function ManageDesignation() {
  ...
  const updateDesignation = (id, designation) => {
    setData(false);
    axios
      .put("http://localhost:8090/api/designations/" + id, designation)
      .then((res) => {
        if (res.status === 200) {
          setEditing(false);
          setPreview(true);
  ...
  const addDesignation = (designation) => {
    setData(false);
    axios
      .post("http://localhost:8090/api/designations/", designation)
      .then((res) => {
        setPreview(true);
        setData(true);
        if (res.status === 200) {
  ...
  const deleteDesignation = (row) => {
    ...
    if (k) {
      axios
        .delete("http://localhost:8090/api/designations/" + row.id)
        .then((res) => {
          setData(true);
          if (res.status === 200) {
            setState({
        ...
        <div className="container-fluid">
          <div className="inner-body">
            ...
            <Grid container>
              ...
              >
                <Typography variant="h6">Manage Designations</Typography>
                {preview ? (
                  <Typography variant="body2">
                    Masters / Manage Designations / Designations List
                  </Typography>
                ) : editing ? (
                  <Typography variant="body2">
                    Masters / Manage Designations / Edit Designation
                  </Typography>
                ) : (
                  <Typography variant="body2">
                    Masters / Manage Designations / Add Designations
                  </Typography>
                )}
              </Grid>
              <Grid
            ...
            <Grid container>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                {preview ? (
                  ...
                  <AddDesignation addDesignation={addDesignation} />
                )}
              </Grid>
            </Grid>
          </div>
